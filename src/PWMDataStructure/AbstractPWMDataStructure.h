...
namespace PWM {
	namespace PWMDataStructure{
		...
		template <typename T> class AbstractPWMDataStructure{
            ...
            public:
                ...
                bool weakComparison(const AbstractPWMDataStructure & other, float precisionThreshold = 0.00001) const;
                bool operator!=(const AbstractPWMDataStructure & other) const;
                AbstractPWMDataStructure& operator+=(const AbstractPWMDataStructure & other);
                AbstractPWMDataStructure& operator*=(const T& val);
    ...
    inline bool AbstractPWMDataStructure<T>::operator==(const AbstractPWMDataStructure<T> & other) const {
        ...
        template<typename T>
        inline bool AbstractPWMDataStructure<T>::weakComparison(const AbstractPWMDataStructure &other, float precisionThreshold) const{
            if (datasize != other.datasize)
                return false;
            for (int i = 0; i < datasize; ++i){
                T val1 = (int) (data[i] * precisionThreshold) / precisionThreshold;
                T val2 = (int) (other.data[i] * precisionThreshold) / precisionThreshold;
                if (val1 != val2)
                    return false;
            }
            return true;
